...
/**
 * Purpose: Auto-scaling manifest generator with humanoid-relative scaling and clamping (now includes cars).
 * Key features: Fully automated manifest + scaling generation with new cars folder.
 * MIT License: https://github.com/[new-repo-path]/LICENSE
 * Timestamp: 2025-06-25 11:50 | File: AutoScalingManifestGenerator_HumanoidRelative_Clamped.js
 */
    ...
    for (const folder of SUBFOLDERS) {
        ...
        for (const file of files) {
            ...
            if (rawHeight > MAX_HEIGHT) {
                console.warn(`[VRBox] Extreme height for ${relPath}: ${rawHeight.toFixed(2)}m, clamped to ${MAX_HEIGHT}m`);
                rawHeight = MAX_HEIGHT;
            }

            if (rawHeight < MIN_HEIGHT) {
                console.warn(`[VRBox] Tiny height for ${relPath}: ${rawHeight.toFixed(2)}m, clamped to ${MIN_HEIGHT}m`);
