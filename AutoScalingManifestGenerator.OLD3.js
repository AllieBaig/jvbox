...
/**
 * Purpose: Fully compatible manifest + scaling generator for glTF-Transform v4.x.
 * Key features: Uses MathUtils API for bounds computation.
 * Dependencies: @gltf-transform/core v4.x
 * MIT License: https://github.com/[new-repo-path]/LICENSE
 * Timestamp: 2025-06-22 14:30 | File: AutoScalingManifestGenerator.js
 */
...
const STANDARD_HEIGHT = 3; // meters normalized target
...
async function generateManifestAndScaling() {
    ...
    for (const folder of SUBFOLDERS) {
        ...
        for (const file of files) {
            ...
            const min = MathUtils.createVec3(Infinity, Infinity, Infinity);
            const max = MathUtils.createVec3(-Infinity, -Infinity, -Infinity);

            for (const mesh of meshes) {
                for (const prim of mesh.listPrimitives()) {
                    const bounds = prim.computeBounds();
                    MathUtils.min(min, min, bounds.min);
                    MathUtils.max(max, max, bounds.max);
                }
            }
